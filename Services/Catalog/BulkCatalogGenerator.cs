...
internal static class BulkCatalogGenerator
{
    ...
    public static IEnumerable<CatalogProperty> GenerateAll()
    {
        ...
        foreach (var state in UsRentalMarkets.All)
        {
            ...
            foreach (var property in GenerateForState(state.Cities, stateTag, seedOffset, UsRentalMarkets.ListingsPerState))
            {
        ...
        int perStateTarget)
    {
        ...
        foreach (var city in cities)
        {
            for (var unit = 1; unit <= perCity; unit++)
            {
                ...
                var slug = $"rental-us-{stateTag}-{city.SlugKey}-{unit:D4}";
                var detail =
